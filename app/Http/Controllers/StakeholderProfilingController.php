...
class StakeholderProfilingController extends Controller
{
    ...
    public function show(int $id)
    {
        $stakeholder = Stakeholder::with(['stakeholderProfiling' => function ($query) {
            $query->orderBy('year', 'desc')
            ->select('id', 'stakeholder_id', 'year', 'stakeholder_kuadran_id', 'power_point', 'interest_point')
            ->with('stakeholderKuadran:id,name');
        }])
        ->with('stakeholderCategory:id,name')
        ->with('stakeholderHasProvince.province:id,name')
        ->with('stakeholderHasCity.city:id,name')
        ->with('stakeholderHasDistrict.district:id,name')
        ->with('stakeholderHasVillage.village:id,name')
        ->find($id, ['id', 'name', 'phone_number', 'stakeholder_category_id']);

        return response()->json($stakeholder);
    }

    public function getProfiling(int $id)
    {
        $stakeholderProfilingDetail = StakeholderProfilingDetail::with('stakeholderProfiling')->with('stakeholderProfiling.stakeholderKuadran')->where('stakeholder_profiling_id', $id)->first();

        return response()->json($stakeholderProfilingDetail);
    }
    ...
    {
        $stakeholderProfiling = StakeholderProfiling::find($id);
        ...
        $stakeholderProfiling = StakeholderProfiling::where('id', $id)->update([
            'year' => $request->year,
            'stakeholder_kuadran_id' => $kuadran,
            'power_point' => $power_point,
            'interest_point' => $interest_point,
        ...
        $sp = StakeholderProfiling::find($id);

        $stakeholderProfilingDetail = StakeholderProfilingDetail::
        where('stakeholder_profiling_id', $sp->id)
        ->update([
            'k_s1_q1' => $request->question1,
            'k_s1_q2' => $request->question2,
            'k_s1_q3' => $request->question3,
        ...
        if ($stakeholderProfiling && $stakeholderProfilingDetail) {
            return response()->json(['message' => 'Stakeholder Profiling Successfully Updated!'], 201);
        }else{
            return response()->json(['message' => 'Stakeholder Profiling Failed Updated!'], 202);
        }
    }

    public function destroy(int $id){
        $stakeholderProfiling = StakeholderProfiling::find($id);
        $stakeholderProfilingDetail = StakeholderProfilingDetail::where('stakeholder_profiling_id', $id)->delete();

        if ($stakeholderProfiling->delete() && $stakeholderProfilingDetail) {
            return response()->json(['message' => 'Stakeholder Profiling Successfully Deleted!'], 201);
        }else{
            return response()->json(['message' => 'Stakeholder Profiling Failed Deleted!'], 202);
        }
    }
}
